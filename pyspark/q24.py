...
# 
cassandra_table_name       = 'arrival_delay_by_route'
cassandra_table_key_name   = 'route'
cassandra_table_value_name = 'arrival_delay'
top = []
...
ssc.checkpoint(args.hdfs_prefix + '/checkpoint/q24')

dstream = ssc.textFileStream(args.hdfs_prefix + args.input_dir)
dstream = dstream.flatMap(extract_route_arr_delay).reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1])).foreachRDD(route_delay)
