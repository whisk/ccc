...
# 
cassandra_table_name       = 'top_carriers_by_route'
cassandra_table_key_name   = 'route'
cassandra_table_value_name = 'carriers'
top = []
...
sc = SparkContext(appName='Top Carriers By route')
ssc = StreamingContext(sc, args.batch_interval)
ssc.checkpoint(args.hdfs_prefix + '/checkpoint/q23')

dstream = ssc.textFileStream(args.hdfs_prefix + args.input_dir)
dstream = dstream.flatMap(extract_route_carrier_arr_delay).reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1])).foreachRDD(top_weekdays)
